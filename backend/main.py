...
@app.get("/health")
async def health():
    """Health check — verifies LLM availability; vector store issues are non-critical."""
    warnings = []

    # Vector store — degraded warning only, does NOT block healthy status
    try:
        from backend.services.vector_store import vector_store
        if vector_store is None:
            warnings.append("vector_store: unavailable (job matching degraded)")
        else:
            vector_store.get_stats()
    except Exception as e:
        warnings.append(f"vector_store: {e} (job matching degraded)")

    # At least one LLM provider — this is critical
    llm_ok = True
    ...
    if not llm_ok:
        return JSONResponse(
            status_code=503,
            content={"status": "degraded", "version": "3.2.0", "warnings": warnings, "issues": ["llm: no providers available"]},
        )

    return {"status": "healthy", "version": "3.2.0", "warnings": warnings}
