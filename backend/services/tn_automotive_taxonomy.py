...
NSQF_LEVELS = {
    1 : {"label" : "NSQF Level 1", "description" : "Basic awareness, no prior skill needed",
         "equivalent" : "Below Class 8"},
    2 : {"label" : "NSQF Level 2", "description" : "Semi-skilled worker, supervised tasks",
         "equivalent" : "Class 8–10"},
    3 : {"label" : "NSQF Level 3", "description" : "Skilled worker, independent tasks",
         "equivalent" : "Class 10 / ITI 1st year"},
    4 : {"label" : "NSQF Level 4", "description" : "Supervisory / technician level",
         "equivalent" : "ITI Pass / Diploma 1st year"},
    ...
    7 : {"label" : "NSQF Level 7", "description" : "Senior Engineer / Specialist",
         "equivalent" : "M.E. / M.Tech / MBA"},
}
...
class TNAutomotiveSkillExtractor :
    ...
    def get_role_profile ( self, role_name: str ) -> Optional[Dict] :
        """Get detailed profile for a specific role."""
        return self.job_roles.get( role_name )

    def get_role_by_skills ( self, skills: List[str] ) -> List[Dict] :
        """Find roles that match given skills."""
        matched_roles = []
        for role_name, role_data in self.job_roles.items() :
            required_skills = role_data.get( "required_skills", [] )
            match_count = len( [s for s in skills if s in required_skills] )
            if match_count > 0 :
                matched_roles.append( {
                    "role" : role_name,
                    "match_count" : match_count,
                    "total_required" : len( required_skills ),
                    "match_percent" : round( match_count / len( required_skills ) * 100, 1 ),
                    "role_data" : role_data
                } )
        return sorted( matched_roles, key=lambda x : x["match_percent"], reverse=True )
